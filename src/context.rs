//Injective encoding L || |c1| || c1 || |c2| || c2
pub fn encode(label: u64, c1: &[u8], c2: &[u8]) -> Vec<u8> {
    let mut context = Vec::with_capacity(8 + 4 + c1.len() + 4 + c2.len());
    context.extend_from_slice(&label.to_be_bytes());
    // Representation of label L as byte array
